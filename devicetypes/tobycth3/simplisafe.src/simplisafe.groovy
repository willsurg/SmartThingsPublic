...
preferences {
    ...
    input(name: "nest", type: "enum", title: "Nest Integration", required: "true", description: "Integrated with Nest", options: ["no", "yes" ])
}
...
def setState (alState){
    ...
    else if (alState == "away")
    {
        ...
        log.info "exitDelay: ${response.data.exitDelay}"
        state.exitDelay = response.data.exitDelay
        state.exit = "AWAY_COUNT"
        def dtms = new Date().getTime() //Date Object in ms
        log.info "Now: ${dtms}" //Log Current Date/Time in ms
        log.info "Now Plus Exit Delay: ${dtms + (response.data.exitDelay * 1000)}" //Log Current Date/Time + Exit Delay in ms
        runOnce(new Date(dtms + (response.data.exitDelay * 1000) + 1000), poll) //Execute poll in Current + Exit Delay in ms - pad with 1000 ms
        sendEvent(name: "events", value: "Exit Delay: ${response.data.exitDelay} Seconds")
        //David A. Norris 9/8/2018
        //Added response to obtain exitDelay from SS.
        //RunOnce added to poll after designated exitDelay to refresh UI
...
def poll() {
	...
	httpGet ([uri: getAPIUrl("refresh"), headers: state.auth.respAuthHeader, contentType: "application/json; charset=utf-8"]) { response ->
        ...
        //David A. Norris 9/8/2018
        //Added to display Exit Count in Messages during Away Countdown
        if(state.exit != null)
        {
            if(state.exit == "AWAY_COUNT")
            {
            	sendEvent(name: "events", value: "Exit Delay ${state.exitDelay} Seconds")
                state.exit = ""
                state.exitDelay = ""
            }
            else
...
def getAPIUrl(urlType) {
    ...
    else if (urlType == "nest")
    {
    	return "https://api.simplisafe.com/v1/integration/nest/$state.subscriptionId/info"
    }
    else
    {
